...
Ключи не пересылаются в переписке, не передаются аргументами команд (такого флага
в скриптах нет намеренно: аргументы видны в списке процессов и остаются в истории
оболочки) и не выводятся на экран. Ключ вводит владелец в невидимом поле
`setup_key.py`, скрипт проверяет его у сервиса и кладёт в системное хранилище
секретов — или в `.env` с правами 600, закрытый от git. Ассистенту доступен
только факт «подключено».

## Об исходниках карт

Скриншот Яндекс.Карт или Google Maps — их материал, и правила этих сервисов
ограничивают производные работы. Для личного альбома на это смотрят сквозь пальцы,
но если карта пойдёт в публикацию, надёжнее снять исходник с
